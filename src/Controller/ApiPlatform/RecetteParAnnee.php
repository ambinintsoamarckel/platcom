...
class RecetteParAnnee extends AbstractController
{
    ...
    public function __invoke(Commande $data , CommandeRepository $commandeRepository,$id)
    {
        if ($this->security->isGranted('ROLE_ADMIN')) {
            # code...
            return ($commandeRepository->recettesParAnnee($id));
        }
        elseif ($this->security->isGranted('ROLE_SELLER'))
        {
            return ($commandeRepository->recettesParAnnee($this->security->getUser()->getId()));
        }
        else
        {
            $responseData = [
