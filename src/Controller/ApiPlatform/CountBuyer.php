...
class CountBuyer extends AbstractController
{
    ...
    public function __invoke(Commande $data , CommandeRepository $commandeRepository,$id)
    {
        if ($this->security->isGranted('ROLE_ADMIN')) {
            return ($commandeRepository->nombreTotalAcheteurs($id));
       
        }
        elseif ($this->security->isGranted('ROLE_SELLER'))
        {
            return ($commandeRepository->nombreTotalAcheteurs($this->security->getUser()->getId()));
        }
        else
        {
