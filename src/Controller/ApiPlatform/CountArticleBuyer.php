...
class CountArticleBuyer extends AbstractController
{
    ...
    public function __invoke(Commande $data , CommandeRepository $commandeRepository,$id)
    {
        if ($this->security->isGranted('ROLE_ADMIN')) {
            return ($commandeRepository->acheteursEtQuantites($id));
        }
        elseif ($this->security->isGranted('ROLE_SELLER'))
        {
            return ($commandeRepository->acheteursEtQuantites($this->security->getUser()->getId()));
        }
        else
