...
function Tab() {
  ...
  const loadData = async (page=1) => {
    ...
    try {
      const response = await axios.get(`https://127.0.0.1:9000/api/produits?page=${page}&order[createdAt]`,{ headers:authHeader(), withCredentials: true});
      const data = response.data["hydra:member"];
      let abx=(response.data["hydra:member"].length);
      setSize(parseInt(abx));
      if (parseInt(response.data["hydra:view"])) {
    ...
    try {
      ...
      if (searchValue.length >= 3) {
        const response = await axios.get(
          `https://127.0.0.1:9000/api/produits?page=1&nomprod=${searchValue}`,
        ...
        // Clear the grid data if the length of searchValue is less than 3
        setGridData([]);
      }
    } catch (error) {
      console.error("Error fetching data:", error);
        ...
        return modifiedData.length >= 1 ? (
          <Space>
                <Link
                  ...
                  to={{
                    pathname: `/react/Zafy/Produits/Edit/${record.id}`,
      ...
      <div className="container-fluid">
        <div className="row align-items-center">
          <div className="col-md-6 mt-3 order-md-1">
            <Link className="btn btn-outline-secondary btn-user" to="/react/Zafy/Produits/Ajout">
              Add
            </Link>
          </div>
