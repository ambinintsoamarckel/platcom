  ...
  const loadData = () => {
    try {
      ...
      const datatable = data && data.map((ligne, index) => ({
        nomprod: ligne.produit.nomprod,
        quantite: ligne.quantite, // Assurez-vous que la propriété exacte est utilisée
      ...
      const monSousTotal = datatable.map(row => ({
        ...row,
        sousTotal: row.prixunitaire * row.quantite
      }));
    ...
    <>
      ...
      <Modal show={show} onHide={handleClose} backdrop="static" keyboard={false}>
        <Modal.Header closeButton>
          <Modal.Title>Ligne d'Opération</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <div>
