...
const AdDataComponent = ({ onAddData }) => {
  const [newData, setNewData] = useState({ nomprod: '', prixunitaire: 0, quantitecom: 0, quantiteL: 0, produits:'', unite:'' });
...
const handleUnitChange = (selectedOption) => {
  const { value: id, label: prixunitaire } = selectedOption || {};
...
<div>
  ...
  <Select
    ...
    options={optionsPu.map((option) => ({ label: option.prix, value: option["@id"] }))}
  />
