...
import ADataComponent from "./Ed_lnpro";
import { useNavigate, useParams } from 'react-router-dom';
import Select from "react-select";
import Znt from "./Ed_img";
...
function Ed_Prod() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`https://127.0.0.1:9000/api/produits/${id}`,
        {
          headers: {
        ...
        const productData = response.data; // Adjust this based on your API response structure

        // Set data to formData and tableData
        setFormData({
          nomprod: productData.nomprod,
          categorie: productData.categorie,
          description: productData.description,
          limite: productData.limite
        });
        setTableData(productData.unites || []);
      } catch (error) {
  ...
  return (
    <>
      <div className="text-center">
        <h1 className="h4 text-gray-900 mb-4">Edit Produits</h1>
      </div>
    ...
    <form onSubmit={handleFormSubmit}>
      <div>
        ...
        className="btn btn-outline-secondary btn-user"
            ...
            multiple
          />
          <ADataComponent onAddData={handleAddData} />
          <div>
  ...
  <div className="col-md-6">
    <div className="p-5">
      ...
      <Znt photos={selectedFiles} onDelete={handleDeletePhoto} />
    </div>
  </div>
</div>
